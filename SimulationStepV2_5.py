...
def gridbuilder(gridArray):
    ...
    # split our grid array values to into two seperate pairs instead of tuples 
    for i in range(len(gridArray)):
        xco = gridArray[i][0]
        yco = gridArray[i][1]
        # if a box exists just add the particle to the box
        if (xco,yco) in grid:
            grid[xco,yco].append(i)
...
def connect(grid, gridArray,N):
    # great a empty list for each particle to store its nearby particles 
        ...
        for dxco in (-1,0,1):
            for dyco in (-1,0,1):
                ...
                if surr in grid:
                    ...
                    for k in grid[surr]:
                        # stops it from adding a particle from being classed as nearby itself
                        if k != i:
                            partnet[i].append(k)
    return partnet 
